...
type
  ...
  TComponentLoadEx = class(TFmxObject);
  TksRoundRect = class(TRoundRect);
  TksBaseSpeedButton = class(TSpeedButton);

  TksBaseTabControl = class(TComponentLoadEx)
  protected
...
uses CommonExt, SysUtils, FMX.Styles, FMX.Styles.Objects, Math, SystemSoundExt;
...
{$ELSE}
      AImgRect := ABitmapLink.LinkByScale(FImageScale, ModuleIsCpp).SourceRect;
{$ENDIF}
